...
class UnetLogoInsertion(BannerReplacer):
    ...
    def __train_model(self, x_train_path, y_train_path, img_height, img_width, img_channels, model_weights_path):
        ...
        # empty lists for X_train and Y_train
        ...
        # training the model
        self.model.fit(x_train, y_train, validation_split=0.1, epochs=30, callbacks=callbacks)
...
if __name__ == '__main__':
    ...
    logo_insertor.build_model('/Users/oleksandr/Folder/superman/UNet/model_parameters_setting')
    ...
    if source_type == 0:
        ...
        # preproccess (detection and smoothing points)
    ...
    # works with image
        ...
        '''if save_result:
            cv2.imwrite(saving_link, mm)'''
        '''cv2.imshow('Image (press Q to close)', mm)
        key = cv2.waitKey(0)
        if key == 27:
            cv2.destroyAllWindows()'''
